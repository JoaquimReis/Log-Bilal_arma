...
rLB <- function(n, mu) {
  u <- runif(n)
  y <- sapply(u, function(ui) qLB(ui, mu = mu))
  return(y)
}
...
phi <- (0.7 / (0.7 + 24))^(-0.5) - 5
y_seq <- seq(0.001, 0.999, length.out = 1000)
cdf_vals <- 3 * y_seq^(4 * phi) - 2 * y_seq^(6 * phi)

plot(y_seq, cdf_vals, type = "l", col = "blue", main = "CDF Log-Bilal (mu = 0.7)")
