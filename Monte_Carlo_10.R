# Modelo ARMA(1,0) - Apenas Autoregressivo 
...
phi <- 0.2  # AR
#theta <- -0.6  # MA
true_values <- c(alpha, phi)  # Valores reais dos parâmetros
...
system.time({
  for (n in vn) {
    ...
    # Contadores
    calpha <- cphi <- 0  # Contadores para os intervalos de confiança
    ...
    for (i in 1:R) {
      y <- simu.LogBarma(n, phi = phi, theta = NA, alpha = alpha, freq = 12, link = "logit")
